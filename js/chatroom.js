var firebaseConfig = {
  ...
  <i id="leave-group-btn" class="bx bx-log-out leave-group-btn" onclick="leaveGroup(${roomId})" style="color: red;"> ${roomId} </i>
  `
 ...
 mesao = document.querySelector('.send-message-container')
 if (roomId != '111111') {
  mesao.style.visibility = 'visible'
 }
 else {
  document.getElementById('hed').innerHTML =' <i class="bx bxs-pin"  style="color: yellow;">  </i>';
 }
  firebase.auth().onAuthStateChanged((user) => {
    if (user) {
      ...
      if (localStorage.getItem('icon') == 'null') {
        ...
        if (user.displayName == 'Shaurya001' && roomId == '111111') {
          mesao.style.visibility = 'visible'
          
        }
        else if (user.displayName != 'Shaurya001' && roomId == '111111') {
          mesao.style.visibility = 'visible'
         document.getElementById('niss').innerHTML = `<div class="bx bx-lock" id="nias"> ONLY TEACHERS CAN SEND MESSAGES!</div>`
        }
    }
